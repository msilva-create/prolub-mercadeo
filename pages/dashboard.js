...
export default function Dashboard() {
  ...
  const [todosLosSaldos, setTodosLosSaldos] = useState([]) // Agregado
  ...
  async function cargarDatos(distribuidorId) {
    ...
    // Agregado: Carga de visualización global
  ...
  // Agregado: Función para la imagen de Excel
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5]">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* Agregado: Botón y Visual Global */}
        {user?.id === 'central-gulf' && (
          <div className="space-y-6 mb-10">
            <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl flex justify-between items-center shadow-sm">
                <p className="text-sm text-orange-800 font-medium italic">Herramienta de control: Carga los saldos de la imagen de Excel.</p>
                <button onClick={cargarMasivoExcel} className="bg-[#F15A22] text-white font-bold py-2 px-6 rounded-lg shadow-md hover:bg-[#d94e1a] transition-all">🚀 Cargar Saldos Excel</button>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Visualización Global de Saldos Cargados</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-sm font-medium">
                    {todosLosSaldos.map(s => (
                        <div key={s.distribuidor_id} className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                            <p className="text-[9px] font-black text-[#1B3A6B] uppercase truncate">{s.distribuidor_id.replace(/-/g, ' ')}</p>
                            <p className="text-sm font-bold text-gray-700">{formatCOP(s.saldo_disponible)}</p>
                        </div>
                    ))}
                </div>
            </div>
